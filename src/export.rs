...
fn render_table_markdown(database: &Database, table: &Table, md: &mut String) {
    ...
    if let Some(ai) = &table.ai {
        md.push_str("\n#### AI-Generated Context\n\n");
        md.push_str("_This section is generated by dbctx and is not a catalog fact._\n\n");
        md.push_str("**Summary:** ");
        md.push_str(&escape_markdown_paragraph(&ai.summary));
        md.push_str("\n\n");
        if !ai.notes.is_empty() {
            md.push_str("**Notes:**\n");
            for note in &ai.notes {
                md.push_str("- ");
                md.push_str(&escape_markdown_cell(note));
                md.push('\n');
            }
        }
    }
...
mod tests {
    use super::*;
    use crate::model::{
        AiContext, AnalysisFinding, AnalysisKind, Column, DatabaseMetadata, DocumentHeader, Engine,
    ...
    #[test]
    fn schema_json_includes_ai_context_when_present() {
        let dir = tempfile::tempdir().unwrap();
        let mut db = database();
        db.tables[0].ai = Some(AiContext {
            generated: true,
            confidence: 0.91,
        ...
        let table = &schema_json["tables"][0];
        assert!(table["ai"].is_object(), "ai should be an object");
        assert_eq!(table["ai"]["generated"], true);
        assert_eq!(table["ai"]["confidence"], 0.91);
        assert_eq!(table["ai"]["summary"], "AI summary");
        assert_eq!(table["ai"]["notes"][0], "note one");
    }

    #[test]
    fn schema_md_renders_ai_context_when_present() {
        let dir = tempfile::tempdir().unwrap();
        let mut db = database();
        db.tables[0].ai = Some(AiContext {
            generated: true,
        ...
        let contents = fs::read_to_string(dir.path().join("schema.md")).unwrap();
        assert!(contents.contains("#### AI-Generated Context"));
        assert!(contents.contains("This section is generated by dbctx"));
        assert!(contents.contains("AI summary"));
        assert!(contents.contains("note one"));
    }

    #[test]
    fn schema_json_omits_ai_context_when_absent() {
        let dir = tempfile::tempdir().unwrap();
        let mut db = database();
        db.sort();
        ...
        assert!(
            schema_json["tables"][0]["ai"].is_null(),
            "ai should be omitted when absent"
        );
    }
    ...
    mod validation {
        ...
        #[test]
        fn schema_json_with_ai_validates_against_schema_1_0() {
            let dir = tempfile::tempdir().unwrap();
            let mut db = database();
            db.tables[0].ai = Some(AiContext {
                generated: true,
        ...
        #[test]
        fn table_json_with_ai_validates_against_table_1_0() {
            let dir = tempfile::tempdir().unwrap();
            let mut db = database();
            db.tables[0].ai = Some(AiContext {
                generated: true,
