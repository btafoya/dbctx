#!/usr/bin/env bash
# MySQL custom entrypoint for the dbctx database lab.
# Copies development TLS material into a writable location and delegates to the
# official MySQL entrypoint with the correct SSL configuration.
set -euo pipefail

ENABLE_TLS="${ENABLE_TLS:-false}"
SRC_TLS_DIR="/etc/mysql/tls"
DST_TLS_DIR="/tmp/mysql-tls"
...
if [[ "${ENABLE_TLS}" == "true" || "${ENABLE_TLS}" == "1" || "${ENABLE_TLS}" == "ON" ]]; then
    if [[ -f "${DST_TLS_DIR}/server.crt" ]]; then
        ...
        exec /usr/local/bin/docker-entrypoint.sh \
            ...
            --require-secure-transport="${MYSQL_REQUIRE_TLS:-OFF}"
    fi
